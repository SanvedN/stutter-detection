...
class TranscriptionAnalyzer:
   def __init__(self, model_size: str = "medium"):
       ...
       try:
           ...
           self.speech_patterns = {
               ...
               'starters': ['basically', 'actually', 'literally', 'obviously', 'frankly'],
           ...
           logger.info(f"TranscriptionAnalyzer initialized with {model_size} model on {self.device}")
       ...
       for category, patterns in self.speech_patterns.items():
           if isinstance(patterns, dict):
               self.pattern_regexes[category] = {
                   'single': re.compile(r'\b(' + '|'.join(patterns['single']) + r')\b', re.IGNORECASE),
                   'compound': re.compile(r'\b(' + '|'.join(patterns['compound']) + r')\b', re.IGNORECASE)
               }
           else:
               self.pattern_regexes[category] = re.compile(r'\b(' + '|'.join(patterns) + r')\b', re.IGNORECASE)
   ...
   def _check_double_negatives(self, tagged_tokens, errors: List[Dict]) -> None:
        ...
        try:
            ...
            neg_positions = [i for i, (word, _) in enumerate(tagged_tokens) 
                            if word.lower() in negatives]
            
            for i in range(len(neg_positions) - 1):
                if neg_positions[i+1] - neg_positions[i] < 5:  # Within 5 words
                    errors.append({
                        'text': ' '.join(word for word, _ in tagged_tokens[neg_positions[i]:neg_positions[i+1]+1]),
   ...
   def _create_filler_entry(self, word_info: Dict, category: str, 
                            context: List[Dict], end_time: float = None,
        ...
        while i < len(word_timings) - 1:
            ...
            if len(pattern) > 1:
                repetitions.append({
                    ...
                    'repetition_type': self._classify_repetition_type(pattern),  # Specific repetition type
                    'confidence': np.mean([word_timings[k]['confidence'] for k in range(i, j)])
                })
                i = j
            else:
                i += 1
   ...
   def _is_grammatical_usage(self, word: str, prev_words: List[str], 
                           next_words: List[str]) -> bool:
   ...
   def _save_analysis_json(self, result: TranscriptionResult, output_path: Path) -> None:
            ...
            try:
                analysis = {
                    ...
                    'fillers': {
                        'count': len(result.fillers),
                        'details': result.fillers
                    },
                    'repetitions': {
   ...
   def _save_textgrid(self, result: TranscriptionResult, output_path: Path) -> None:
    ...
    try:
        ...
        # Repetitions tier
        textgrid_content += self._create_textgrid_tier("repetitions", result.repetitions, 3)
   ...
   def _calculate_speech_rate(self, word_timings: List[Dict], duration: float) -> float:
       ...
       content_words = [w for w in word_timings 
                       if not w.get('is_partial') 
                       and not self._is_filler(w['word'])]
   ...
   def _calculate_fluency_score(self, doc) -> float:
       ...
       filler_count = len([token for token in doc 
   ...
   def _save_summary_report(self, result: TranscriptionResult, 
...
if __name__ == "__main__":
   try:
       # Create analyzer
       analyzer = TranscriptionAnalyzer(model_size="medium")
